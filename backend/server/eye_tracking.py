...
#         final_output = average_gaze * (99/100) * -1
...
def eye_tracking_method(video_path):
    ...
    while cap.isOpened():
        ...
        frame_count += 1
        if frame_count % 5 != 0:  # Process every 5th frame for performance improvement
            continue
    ...
    if gaze_percentages:
        average_gaze = sum(gaze_percentages) / len(gaze_percentages)
        final_output = (100-100+average_gaze) * 100  # Convert to percentage
        return final_output * 2.5
        # return average_gaze
    else:
        print("No gaze data available.")
        return 0.0
